...
shortname:
	...
	sprintf(temp, "FILE DE change: de->name= %s\n", de->name); //grafoume to onoma
	...
	de->size = 0;
	sprintf(temp, "FILE D change :de->size= %d",de->size); //grafoume to megethos
	sys_write(sbi->arxeio_katagrafis, temp , sizeof(temp));
	sys_fsync(sbi->arxeio_katagrafis);
	sys_fdatasync(sbi->arxeio_katagrafis);
				  ...
				  unsigned int flags)
{
	...
	struct dentry *alias;
	int err;
	printk(KERN_INFO "use of vfat_lookup(FOLDER (namei_vfat.c))\n");//eidame vfatlook up
...
static int vfat_create(struct inode *dir, struct dentry *dentry, umode_t mode,
		       bool excl)
{
	printk(KERN_INFO "use of vfat_create(FOLDER (namei_vfat.c))\n");//eidame vfat create
	struct super_block *sb = dir->i_sb;
	struct inode *inode;
	struct fat_slot_info sinfo;
	struct timespec ts;
...
{
	printk(KERN_INFO "use of vfat_rmdir(FOLDER (namei_vfat.c))\n");//eidame vfat m dir
...
{
	printk(KERN_INFO "use of vfat_unlink(FOLDER (namei_vfat.c))\n");//eidame vfat unlink
	struct inode *inode = d_inode(dentry);
	struct super_block *sb = dir->i_sb;
	struct fat_slot_info sinfo;
	int err;
...
static int vfat_mkdir(struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	ts = current_time(dir);
	printk(KERN_INFO "use of vfat_mkdir(FOLDER (namei_vfat.c))\n");//eidame vfatmkdir
...
{	
	...
	printk(KERN_INFO "use of vfat_rename(FOLDER (namei_vfat.c))\n");
	//char *temp = kzalloc(sizeof(*slots), GFP_KERNEL);
